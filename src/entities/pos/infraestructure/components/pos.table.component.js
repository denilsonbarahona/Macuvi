    ...
    return (
            <div className="grid grid-cols-12 gap-6 mt-5">                
                <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">                            
                    <div className="hidden md:block mx-auto text-gray-600">{state.props.size} POS como resultado de la búsqueda</div>                            
                </div>    
                    {(state.props.array !== undefined && state.props.array.length>0)
                        ?<div className="intro-y col-span-12 overflow-auto lg:overflow-visible">
                            <table className="table table-report -mt-2">
                                <thead>
                                    <tr>
                                        <th className="whitespace-nowrap">NOMBRE DEL POS</th> 
                                        <th className="whitespace-nowrap">% DE COBRO</th> 
                                    ...
                                    {state.props.array !== undefined && state.props.array[state.props.page] !== undefined &&
                                        state.props.array[state.props.page].map((item,index)=>(
                                            <tr key={index} className="intro-x">
                                                <td className="W-40"> {item.data.PosName} </td>  
                                                <td className="W-40"> {item.data.PosFee} % </td>                                                       
                                                <td className="table-report__action w-56">
                                                    <div className="flex justify-center items-center">
                                                        <div onClick={()=>{ state.props.onEditPOS(item.id) }} className="flex items-center mr-3 cursor-pointer"> <i data-feather="eye" className="w-4 h-4 mr-1" /> Visualizar </div> 
                                                        <div onClick={()=>{ state.props.deletePOS(item.id) }} className="flex items-center mr-3 cursor-pointer text-theme-6"> <i data-feather="trash-2" className="w-4 h-4 mr-1" /> Eliminar </div>                                                                                                                             
                                                    </div>
                                                </td>
                                            </tr>  
                                        ))}
                    ...
                    {(state.props.array !== undefined && state.props.array.length>0)
                        ?<div className="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
                            <ul className="pagination">
                                ...
                                <li onClick={()=>{ state.props.setPager(state.props.array.length -1)}}  > 
                                    <div className="pagination__link cursor-pointer" > <i className="w-4 h-4" data-feather="chevrons-right" /> </div> 
                                </li>
                            </ul>                   
                        </div>
                        :<></> }
