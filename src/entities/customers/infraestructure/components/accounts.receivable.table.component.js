                        ...
                        <table className="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th className="whitespace-nowrap">TOTAL DEL CRÉDITO</th>  
                                    ...
                                    {state.props.array !== undefined && state.props.array[state.props.page] !== undefined &&
                                        state.props.array[state.props.page].map((item, index)=>(
                                            <tr key={index} className="intro-x">
                                                <td className="W-40"> {item.data.AccountsReceivableCustomer.idNumber} </td> 
                                                <td className="W-40"> {item.data.AccountsReceivableCustomer.Name} </td>  
                                                <td className="W-40"> L {Number(item.data.AccountsReceivableTotalDebt).toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} </td>                                                       
                                                <td className="table-report__action w-56">
                                                    <div className="flex justify-center items-center">
                                                        <div onClick={()=>{ state.props.onShowReceivable(item.id)  }} className="flex items-center mr-3 cursor-pointer"> <i data-feather="eye" className="w-4 h-4 mr-1" /> Visualizar </div> 
                                                        <div onClick={()=>{ state.props.onDeleteReceivable(item.id)  }} className="flex items-center mr-3 cursor-pointer text-theme-6"> <i data-feather="trash-2" className="w-4 h-4 mr-1" /> Eliminar </div>                                                                                                                             
                                                    </div>
                                                </td>
                                            </tr>
                                        )) }                                                                                                                                                                  
