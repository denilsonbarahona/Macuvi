                        ...
                        <table className="table table-report -mt-2">
                            <thead>
                                <tr>
                                    ...
                                    <th className="whitespace-nowrap">TIPO DE USUARIO</th>   
                                ...
                                {state.props.array !== undefined && state.props.array[state.props.page] !== undefined && 
                                    state.props.array[state.props.page].map((item, index)=>(
                                        <tr  key={index} className="intro-x">
                                            <td className="W-40"> {item.data.AuthUserName} </td> 
                                            <td className="W-40"> {item.data.AuthUserEmail} </td> 
                                            <td className="W-40"> {item.data.AuthIsMasterUser?"ADMINISTRADOR":"COLABORADOR"} </td>
                                            <td className="W-40"> {item.data.AuthActivatedState===1?"ACTIVO"
                                                                    :item.data.AuthActivatedState===2?"ESPERANDO REGISTRO"
                                                                    :"SUSPENDIDO"} </td>                                                                                      
                                            <td className="table-report__action w-56">
                                                <div className="flex justify-center items-center">
                                                    <div onClick={()=>{ state.props.onEditProfile(item.id) }} 
                                                         className="flex items-center mr-3 cursor-pointer"> 
                                                            <i data-feather="eye" className="w-4 h-4 mr-1" /> Visualizar </div> 
                                                    <div onClick={()=>{ state.props.onDeleteProfile(item.id, item.data.AuthUserFirebaseUi, item.data.AuthIsMasterUser) }} 
                                                        className="flex items-center mr-3 cursor-pointer text-theme-6"> 
                                                            <i data-feather="trash-2" className="w-4 h-4 mr-1" /> Eliminar 
                                                    </div>                                                                                                                             
                                                </div>
                                            </td>
                                        </tr>
