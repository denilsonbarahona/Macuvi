...
const RequestNotificationSuccess =()=>{
    ...
    const [notifyState, ] = useState({email:params.email, required:["email"] })
 
    const onResentClick=()=>{
        dispatch(requestChangePassword({...notifyState}))
    }
                ...
                <div className="block xl:grid grid-cols-2 gap-4">
                    ...
                    <div className="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
                        <div className="my-auto mx-auto xl:ml-20 bg-white xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none sm:w-3/4 lg:w-2/4 xl:w-4/5">
                            ...
                            {login.response.show
                                ?<div 
                                    className={ "intro-x flex text-theme-9 text-xs sm:text-sm mt-4" }>                                
                                    <span>Hemos reenviado el correo para que puedas realizar el cambio de tu contraseña</span> 
                                 </div>
                                :<></> }
                                    
                            <div className="intro-x mt-4">
                                <div className="intro-x mt-2 text-gray-500 justify-start justify-end">
                                    Hemos enviado un correo electrónico a <b>{params.email}</b>. Haz click en el enlace del correo electrónico para restablecer tu contraseña. 
                                    Si no vez el correo electrónico, verifica otros lugares en los que podría estar, como su correo basura o spam. 
                                </div>
                            </div>
                            <div className="intro-x mt-5 xl:mt-8 text-center xl:text-left">
                                <>
                                    <button style={{display: !ui.button?"none":""}} 
                                        ...
                                        Re-enviando correo <img className="h-5 xl:ml-6 ml-20" alt="" src="/assets/images/white.gif"></img>
                                    </button>
                                    <button style={{display: !ui.button?"":"none"}} onClick={()=>{ onResentClick() }} 
...
export default RequestNotificationSuccess;
