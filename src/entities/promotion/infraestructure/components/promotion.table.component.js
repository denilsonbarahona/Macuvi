...
const TablePromotion =(props)=>{
    ...
    useEffect(()=>{
        if(props.payload.promotions.array !== undefined){
    ...
    return( <div className="grid grid-cols-12 gap-6 mt-5"> 
                 {(!props.payload.isLoaded)
                    ...
                    :<div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">                                         
                        <div className="hidden md:block mx-auto text-gray-600">{props.payload.promotions.size} Cantidad de descuentos como resultado de la búsqueda</div>                                                                    
                    </div> } 
                <div className="intro-y col-span-12 overflow-auto lg:overflow-visible">
                    {(!props.payload.isLoaded)
                        ...
                        :<table className="table table-report -mt-2">
                            ...
                            <tbody>                                 
                                {props.payload.promotions.array !== undefined && props.payload.promotions.array[props.payload.Page] !== undefined && 
                                    props.payload.promotions.array[props.payload.Page].map((promotion , index)=>(

                                    <tr key={index} className="intro-x">
                                        <td>{promotion.data.PromotionName}</td>
                                        <td className="text-center">{
                                            (promotion.data.PromotionEndingDate==="")
                                                ?"Sin fecha de finalización"
                                                :new Date(promotion.data.PromotionEndingDate).toLocaleDateString([],{ year: 'numeric', month: 'long', day: 'numeric' }) 
                                        }</td>
                                        <td className="table-report__action w-56">
                                            <div className="flex justify-center items-center">
                                                <div onClick={()=>{ props.payload.showPromotion(promotion.id) }} className="flex items-center mr-3 cursor-pointer"> 
                                                    <i data-feather="check-square" className="w-4 h-4 mr-1"></i> Modificar 
                                                </div>
                                                <div onClick={()=>{ props.payload.onDeletePromotion(promotion.id) }} className="flex items-center text-theme-6 cursor-pointer"> 
                ...
                <div className="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
                    {(props.payload.isLoaded)
                        ?<ul className="pagination">
                            ...
                            {(props.payload.promotions.array !== undefined && props.payload.promotions.array.map((item, index)=>(                                            
                            ...
                            <li  onClick={()=>{ if(props.payload.Page  < props.payload.promotions.array.length - 1)
                                                    props.payload.SetPage(props.payload.Page+1)
                                            }} > 
                                <div className="pagination__link" > <i className="w-4 h-4" data-feather="chevron-right" /> </div> 
                            </li> 
                            <li  onClick={()=>{ props.payload.SetPage(props.payload.promotions.array.length -1) }} > 
