...
import { GetProductById,
         UpdateProductInformation } from '../../core/actions/products.actions';
...
const ProductEdit =()=> 
{
    ...
    },[dispatch,loading.loading, categories.find, cookie.find, cookie.cookie.login.companyId])

    useEffect(()=>{
        if(!loading.loading && !cookie.find && !categories.find && taxes.getTaxes){
            dispatch(GetTaxes(cookie.cookie.login.companyId))
        }
    },[dispatch, cookie.cookie.login.companyId, loading.loading, categories.find, cookie.find, taxes.getTaxes])

    useEffect(()=>{
        if( !cookie.find && !categories.find && !taxes.getTaxes){        
            window.setMenu();       
            dispatch(GetProductById({id: params.id, onSetProductState: onSetProductState}))
        }
    },[dispatch, cookie.find, categories.find, taxes.getTaxes, params.id])

    const onSetProductState=(product)=>{  
        window.feather.replace();         
        setProduct({
            id      : product.id,
            file    : "",
            folder  : product.data.productImgFolderRef,
            photo   : product.data.productImg,
            name    : product.data.productName,
            type    : product.data.productType,
            category: product.data.productCategory,
            tax     : product.data.productTax,
            include : product.data.productTaxIncluded,
            hasVari : product.data.productHasVariations
        })
        setVariations({ items: [...product.data.productVariations], 
            variationsType: (product.data.productVariations[0]===undefined)
              ?"" 
              :product.data.productVariations[0].data.type })
    }
                ...
                <div className="flex">
                    ...
                    <div className="content">
                        ...
                        <div className="intro-y flex flex-col sm:flex-row items-center mt-8">
                            <h2 className="text-lg font-medium mr-auto"><b>Información del producto</b></h2>  
                            <div className="w-full sm:w-auto flex mt-4 sm:mt-0">                            
                                <button style={{display:(loading.button)?"":"none" }} className="button w-44 text-white bg-theme-1 inline-flex items-center ml-auto"> Actualizando ...<img className="h-5 ml-8" alt="" src="/assets/images/white.gif"></img> </button>
                                <button style={{display:(loading.button)?"none":"" }}
                                        onClick={ onClickUpdateProduct } 
                                        className="button w-44 text-white bg-theme-1 shadow-md ml-auto">Actualizar información</button>
                            </div>                                                                                                 
                        </div> 
                        {(products.Updateresponse.show_response)
                            ?<div className={(products.Updateresponse.response)
